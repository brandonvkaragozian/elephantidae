...
ROAD_AVOIDANCE_KM = 0.8                 # Collision hotspot radius (conservative for small reserve)
    ...
    try:
        ...
        for placemark in root.findall('.//kml:Placemark', ns):
            ...
            if 'water' in name or 'lake' in name or 'pond' in name or 'malampuzha' in name:
                ...
                # Only major roads (highways, railways) to avoid over-representation in small area
                cat = 'road'
            else:
                continue
                    ...
                    for coord_str in coords_elem.text.strip().split():
                        ...
                        if len(parts) >= 2:
                            try:
                                ...
                                pass
                    if coords_list:
                        # For roads, use midpoint as single representative location
                        # Avoids over-representation while maintaining road presence
                        coords = np.mean(coords_list, axis=0)
...
def is_movement_toward_resource(point, direction_point, resources, resource_threshold_km=3.0):
    """
    Check if trajectory segment is moving toward a resource (water/crops).
    
    Parameters:
    - point: current point [lon, lat]
    - direction_point: next point [lon, lat] to infer direction
    - resources: array of resource locations
    - resource_threshold_km: proximity threshold to consider resource relevant
    
    Returns: True if moving closer to any nearby resource
    """
    if len(resources) == 0:
        return False
    
    # Find closest resource(s) within threshold
    ...
    # Only consider if already near a resource (within threshold)
    ...
    # Moving toward if distance decreased
...
def evaluate_multi_constraints(traj_walayar, features, time_of_day_fraction=None):
    ...
    Includes context-aware road crossing logic for strategic resource access.
    ...
    # Constraint 4: Roads avoided (with context-aware crossing logic)
    # Elephants avoid roads UNLESS crossing to access water/cropfields
    if len(features['road']) > 0:
        road_crossing_zone = 0.5  # km threshold to detect crossing
        ...
        for i in range(len(traj_walayar)):
            ...
            # If within crossing zone
            if min_road_dist < road_crossing_zone:
                total_road_violations += 1
                
                # Check if movement is justified (heading toward resource)
                if i < len(traj_walayar) - 1:
                    next_point = traj_walayar[i + 1]
                    
                    # Is elephant moving toward water or cropfields?
                    ...
                    # If NOT moving toward resources, it's an unjustified crossing
                    if not (toward_water or toward_crops):
                        unjustified_crossings += 1
                else:
                    # End of trajectory near road without clear direction
                    unjustified_crossings += 1
        
        # Fail road constraint only if MAJORITY of road encounters are unjustified
        if total_road_violations > 0:
            justified_ratio = (total_road_violations - unjustified_crossings) / total_road_violations
            if justified_ratio < 0.5:  # >50% of crossings must be justified
